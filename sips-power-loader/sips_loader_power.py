"""
  ...
  ============================================
  ETL que extrae consumos de electricidad desde una API SIPS y los carga
...
API_POWER_URL = "/api/v1/SIPS/ELE/GetClientesPost" # PARA POWER
...
QUERY_CUPS_ACTIVOS = """
    -- Query que devuelve una columna con los códigos CUPS activos de electricidad.
    -- Debe filtrar por estado del contrato y tipo de suministro (power).
    -- Ejemplo: SELECT cups_codigo FROM contratos WHERE estado IN (...) AND tipo = 'power'
"""

MERGE_CONSUMOS_SQL = """
    -- MERGE (upsert) que inserta o actualiza los consumos de electricidad.
    -- Clave primaria: CUPS + fecha inicio.
    -- Parámetros (en orden): cups, fec_ini, fec_fin, tarifa,
    --   activa_p1..p6, reactiva_p1..p6, potencia_p1..p6,
    --   codigo_equipo_medida, codigo_tipo_lectura
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            f"sips_loader_power_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log",
            encoding='utf-8'
        )
    ]
)
...
def get_cups_power(cursor):
    """Recupera CUPS activos de Power (electricidad) ejecutando la query configurada."""
    cursor.execute(QUERY_CUPS_ACTIVOS)
    cups_list = [row[0] for row in cursor.fetchall()]
    logger.info(f"CUPS Power activos encontrados: {len(cups_list)}")
    return cups_list
...
def call_api(token, url, cups_code):
    ...
    Devuelve el JSON parseado, "TOKEN_EXPIRED" si recibe 401,
    o None si falla tras los reintentos.
    """
    headers = {
        'Authorization': token,
...
# ==============================================================================
# PROCESAMIENTO E INSERCIÓN - POWER (ELECTRICIDAD)
# ==============================================================================
...
def process_power_consumos(cursor, cups_code, json_data):
    """
    Procesa los consumos de electricidad de un CUPS y los inserta/actualiza
    en la tabla destino mediante MERGE.
    Devuelve el número de registros procesados.

    Campos extraídos del JSON de la API SIPS (Power):
      - FechaInicio / FechaFin                     --> fechas del periodo
      - CodigoTarifaATR                            --> tarifa ATR
      - Activa1..Activa6                           --> consumo activa por periodo (6 periodos)
      - Reactiva1..Reactiva6                       --> consumo reactiva por periodo (6 periodos)
      - Potencia1..Potencia6                       --> potencia por periodo (6 periodos)
    ...
    count = 0
    for c in consumos:
        # Para Power, usamos FechaInicio / FechaFin
        fec_ini = parse_date(c.get("FechaInicio"))
        fec_fin = parse_date(c.get("FechaFin"))
        ...
        params = (
            ...
            fec_fin,                                     # Fecha fin
            c.get("CodigoTarifaATR"),                    # Tarifa
            safe_numeric(c.get("Activa1")),              # Consumo activa P1
            safe_numeric(c.get("Activa2")),              # Consumo activa P2
            safe_numeric(c.get("Activa3")),              # Consumo activa P3
            safe_numeric(c.get("Activa4")),              # Consumo activa P4
            safe_numeric(c.get("Activa5")),              # Consumo activa P5
            safe_numeric(c.get("Activa6")),              # Consumo activa P6
            safe_numeric(c.get("Reactiva1")),            # Consumo reactiva P1
            safe_numeric(c.get("Reactiva2")),            # Consumo reactiva P2
            safe_numeric(c.get("Reactiva3")),            # Consumo reactiva P3
            safe_numeric(c.get("Reactiva4")),            # Consumo reactiva P4
            safe_numeric(c.get("Reactiva5")),            # Consumo reactiva P5
            safe_numeric(c.get("Reactiva6")),            # Consumo reactiva P6
            safe_numeric(c.get("Potencia1")),            # Potencia P1
            safe_numeric(c.get("Potencia2")),            # Potencia P2
            safe_numeric(c.get("Potencia3")),            # Potencia P3
            safe_numeric(c.get("Potencia4")),            # Potencia P4
            safe_numeric(c.get("Potencia5")),            # Potencia P5
            safe_numeric(c.get("Potencia6")),            # Potencia P6
            c.get("CodigoDHEquipoDeMedida"),              # Código equipo medida
            c.get("CodigoTipoLectura"),                   # Tipo lectura
        )
...
def process_sector(cursor, conn_db, token, cups_list, api_url, sector_name, process_func):
    ...
    Devuelve (cups_ok, cups_sin_datos, cups_error, total_registros, token)
    para que el token renovado se pueda reutilizar.
    """
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("INICIO PROCESO SIPS CONSUMOS LOADER - POWER")
    ...
    try:
        ...
        # 3. Procesar POWER (Electricidad)
        cups_power = get_cups_power(cursor)
        if cups_power:
            process_sector(
                cursor, conn_db, token,
                cups_power, API_POWER_URL,
                "POWER", process_power_consumos
            )
        else:
            logger.warning("No se encontraron CUPS activos de Power.")
